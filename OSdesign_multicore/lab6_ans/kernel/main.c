...
boot_aps(void)
{
	...
	void *mpentrykva = KADDR(MPENTRY_PADDR);

	memmove(mpentrykva, mpentry_start, mpentry_end - mpentry_start);
	...
	for(cpuno = 0; cpuno < ncpu; cpuno++)
	{
		...
		mpentry_kstack = percpu_kstacks[cpuno] + KSTKSIZE;
		lapic_startap(cpus[cpuno].cpu_id, PADDR(mpentrykva));

		while(cpus[cpuno].cpu_status != CPU_STARTED);
	}
