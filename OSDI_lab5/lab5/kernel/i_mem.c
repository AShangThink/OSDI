...
page_free_list = NULL;
	...
	num_free_pages = 0;
	for (i = 0; i < npages; i++) {
		if (i == 0 || (npages_basemem <= i && i < npages_basemem + num_pages_io_hole) || (npages_basemem + num_pages_io_hole <= i && i <  num_kpages)) {
			pages[i].pp_ref = 1;
...
struct PageInfo *pp;
	if (page_free_list == NULL)
  	...
  	pp = page_free_list;//pp is PA condition 
  	//assert(pp->pp_ref == 0);
  	page_free_list = pp->pp_link;
  	pp->pp_link = NULL;

	if (alloc_flags & ALLOC_ZERO)//ALLOC_ZERO:mem.h
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	while (current_va < end_va) {
		...
		*pte = (current_pa | perm | PTE_P);//PT[i] get "PPN + Flags"
		if (current_va == last_page_addr)
			break;

		current_va += PGSIZE;//reserve a place for current page, idx to next point
		current_pa += PGSIZE;//the same
  	}
}
...
{
//	boot_map_region(pgdir, start, ROUNDUP(size, PGSIZE), PADDR((void*)start), PTE_W | PTE_U);
	//boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm);//no return
	//pde_t                    *kern_pgdir;		// Kernel's initial page directory
	//struct PageInfo          *pages;		// Physical page state array
	//static struct PageInfo   *page_free_list;	// Free list of physical pages
	//size_t                   num_free_pages;
	...
	//t->pgdir = (pde_t*) page2kva(pp);
	memcpy(this, kern_pgdir, PGSIZE);
		
	this[PDX(UVPT)] = (PADDR(this) | PTE_U | PTE_W | PTE_P);//*pde = (page2pa(new_page) | PTE_P | PTE_U | PTE_W);
	//PADDR(kern_pgdir) | PTE_U | PTE_P
	//kern_pgdir[PDX(UVPT)] = PADDR(kern_pgdir) | PTE_U | PTE_P;
	//printk("setupkvm() down!\n");
	return this;
